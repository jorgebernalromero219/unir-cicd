...
server:
	set -ex
	sudo docker network create calc-server-dev || true
	sleep 1
	sudo docker stop apiserver || true
	sudo docker rm --force apiserver || true
	sudo docker run --rm --network calc-server-dev --name apiserver --network-alias apiserver --env PYTHONPATH=/opt/calc --env FLASK_APP=app.api.py -p 5001:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
...
test-e2e:
	...
	API_CONTAINER_ID=$(sudo docker run -d --network calc-test-e2e --env PYTHONPATH=/opt/calc --name apiserver-e2e -p 5001:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0)
...
deploy-stage:
	...
	sudo docker network rm calc-deploy || true
	sleep 1
	sudo docker network create calc-deploy || true
	sleep 1
	sudo docker run -d --network calc-deploy --name apiserver --network-alias apiserver --env PYTHONPATH=/opt/calc --env FLASK_APP=app.api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	sudo docker run -d --network calc-deploy --name calc-web -p 80:80 calc-web

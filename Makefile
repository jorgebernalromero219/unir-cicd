...
test-api:
	...
	API_CONTAINER_ID_API=$(sudo docker run -d --network calc-test-api --env PYTHONPATH=/opt/calc --name apiserver-api --env FLASK_APP=app.api.py -w /opt/calc calculator-app:latest flask run --host=0.0.0.0)
...
test-e2e:
	...
	API_CONTAINER_ID=$(sudo docker run -d --network calc-test-e2e --env PYTHONPATH=/opt/calc --name apiserver-e2e --env FLASK_APP=app.api.py -w /opt/calc calculator-app:latest flask run --host=0.0.0.0)
